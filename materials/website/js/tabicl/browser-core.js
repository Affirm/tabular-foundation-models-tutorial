/* browser-core.js — client-side forward pass for the TabICL v2 classifier core.
 *
 * Experiment-owned runtime based on the verified nano forward pass. It loads
 * upstream-named classifier checkpoints after in-memory key translation and
 * supports fp32, int8, and fp16 tensor encodings. Batch size 1. No dependencies.
 *
 * Preprocessing (must match training): z-score by TRAIN mean/std, then repeated
 * feature grouping with offsets 2^i = {1,2,4} (circular). RoPE is used only in the
 * row stage; QASSMax only in column tfm1 and the ICL blocks.
 */
...
function dequantize(manifest, buffer) {
  ...
  for (const m of manifest.tensors) {
    ...
    if (m.qtype === "fp32") {
      for (let i = 0; i < numel; i++) data[i] = dv.getFloat32(m.offset + 4 * i, true);
    } else if (m.qtype === "int8") {
    ...
    } else if (m.qtype === "fp16") {
      for (let i = 0; i < numel; i++) data[i] = f16(dv.getUint16(m.offset + 2 * i, true));
    } else {
      throw new Error(`Unsupported tensor encoding: ${m.qtype}`);
    }
...
function tensorData(W, name, fallbackLen = 0) {
  const tensor = W.get(name);
  return tensor ? tensor.data : new Float32Array(fallbackLen);
}
...
function block(W, pfx, q, Lq, kv, Lkv, E, nHeads, opts) {
  const g = tensorData(W, pfx + ".ln_attn.weight", E), b = tensorData(W, pfx + ".ln_attn.bias", E);
  ...
  addInto(xs, attnOut);
  const gm = tensorData(W, pfx + ".ln_mlp.weight", E), bm = tensorData(W, pfx + ".ln_mlp.bias", E);
...
function rowStage(W, C, cols, N, E, trace = null) {
  ...
  const rowLnG = tensorData(W, "row_ln.weight", iclDim), rowLnB = tensorData(W, "row_ln.bias", iclDim);
  ...
  if (ytrain) {
    ...
    for (let c = 0; c < F; c++) {
      ...
      for (let i = 0; i < nTrain; i++) {
        ...
        if (maxC === 0) {
          const y = ytrain[i];
          for (let e = 0; e < E; e++) dst[o + e] += yInW[e] * y + yInB[e];
        } else {
          const cls = ytrain[i];
          for (let e = 0; e < E; e++) dst[o + e] += yInW[e * maxC + cls] + yInB[e];
        }
      }
    }
  }
...
function cachedAttentionWeights(W, pfx, q, E, nHeads, cache, ssmax) {
  ...
  const b = tensorData(W, pfx + ".ln_attn.bias", E);
...
export function buildModel(manifest, buffer) {
  const W = dequantize(manifest, buffer);
  ...
  return {
    ...
    prepareContext(Xtrain, ytrain) {
      ...
      const { mean, std } = C.standardize_input === false
        ? { mean: new Float32Array(F), std: new Float32Array(F).fill(1) }
        : standardizeStats(Xtrain, F, nTrain);
      const Xz = C.standardize_input === false ? Xtrain.map((r) => r.map(Number)) : Xtrain.map((r) => zrow(r, mean, std, F));
      ...
      for (let i = 0; i < nTrain; i++) {
        ...
        if (maxC === 0) {
          const y = ytrain[i];
          for (let e = 0; e < iclDim; e++) rowVec[o + e] += yIcW[e] * y + yIcB[e];
        } else {
          const cls = ytrain[i];
          for (let e = 0; e < iclDim; e++) rowVec[o + e] += yIcW[e * maxC + cls] + yIcB[e];
        }
      }
    ...
    predictQueries(cache, Xtest) {
      const F = cache.F, nTest = Xtest.length, E = C.embed_dim, nCls = C.n_cls_cols, iclDim = E * nCls;
      const outDim = C.max_classes === 0 ? C.num_quantiles : C.max_classes;
      ...
      // out_ln -> out_mlp.
      const olG = tensorData(W, "out_ln.weight", iclDim), olB = tensorData(W, "out_ln.bias", iclDim);
      ...
      const out = [];
      for (let i = 0; i < nTest; i++) { const row = []; for (let k = 0; k < outDim; k++) row.push(logits[i * outDim + k]); out.push(row); }
      return out;
    },
     ...
     */
    inspectQuery(cache, query, nClasses = 3, options = {}) {
      const F = cache.F;
      const E = C.embed_dim;
      ...
      const embedding = sampleColumns(cols, E, full ? E : 24);
      const columnBlocks = [];
      for (let bi = 0; bi < C.col_num_blocks; bi++) {
        for (let column = 0; column < F; column++) {
        ...
        columnBlocks.push({
          block: bi + 1,
          activation: sampleColumns(cols, E, full ? E : 24),
        });
      }
      ...
      let rv = rowStage(W, C, cols, 1, E, (bi, tokens, count, dim) => {
        rowBlocks.push({
          block: bi + 1,
          tokens: sampleMatrix(tokens, count, dim, full ? dim : 24),
        });
      });
      const rowVector = sampleMatrix(rv, 1, iclDim, full ? iclDim : 32);
      ...
      for (let bi = 0; bi < C.icl_num_blocks; bi++) {
        ...
        iclBlocks.push({
          block: bi + 1,
          activation: sampleMatrix(rv, 1, iclDim, full ? iclDim : 32),
          attention,
        });
      }

      const olG = W.get("out_ln.weight").data;
      const olB = tensorData(W, "out_ln.bias", iclDim);
      ...
      return {
        ...
        iclBlocks,
        output: {
          normalized: sampleMatrix(normed, 1, iclDim, full ? iclDim : 32),
          hidden: sampleMatrix(hidden, 1, m0w.shape[0], full ? m0w.shape[0] : 32),
          logits,
          probabilities,
        },
      };
